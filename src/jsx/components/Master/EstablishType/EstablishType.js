...
function EstablishType() {
  ...
  const postData = async (values, resetForm) => {
    await MasterApis.PostEstablishTypes(values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
  ...
  const putData = async (values, resetForm) => {
    await MasterApis.PutEstablishTypes(SelectedId, values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
          setAddDataModel(false);
          getDataHandler();
  ...
  const modalDeleteHandle = async () => {
    await MasterApis.DeleteEstablishTypes(SelectedId)
      .then((resp) => {
        getDataHandler();
        setSelectedId("");
        setShowDelete(false);
...
//   put data prepare
  ...
  return (
    <Fragment>
      <AddHeader btnName="Add Establish Type" addHandler={() => newAddHandler()} />

      <Col lg={12}>
        <Card>
          <Card.Header>
            <Card.Title>Establish Type</Card.Title>
          </Card.Header>
          <Card.Body>
