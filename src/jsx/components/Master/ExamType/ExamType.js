...
function ExamType() {
  ...
  const postData = async (values, resetForm) => {
    await MasterApis.PostExamTypes(values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
  ...
  const putData = async (values, resetForm) => {
    await MasterApis.PutExamTypes(SelectedId, values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
          setAddDataModel(false);
          getDataHandler();
  ...
  const modalDeleteHandle = async () => {
    await MasterApis.DeleteExamTypes(SelectedId)
      .then((resp) => {
        getDataHandler();
        setSelectedId("");
        setShowDelete(false);
...
//   put data prepare
  ...
  return (
    <Fragment>
      <AddHeader btnName="Add Exam Type" addHandler={() => newAddHandler()} />

      <Col lg={12}>
        <Card>
          <Card.Header>
            <Card.Title>Exam Type</Card.Title>
          </Card.Header>
          <Card.Body>
