...
function ApprovalType() {
  ...
  const postData = async (values, resetForm) => {
    ...
    await MasterApis.PostApprovalTypes(form)
      .then((resp) => {
        if (!resp.ok) {
          console.log(resp.originalError);
        }
  ...
  const putData = async (values, resetForm) => {
    alert("Editing Not Available now. For this group!")
    // await MasterApis.PutApprovalTypes(id, form)
    //   .then((resp) => {
    //     if (resp.ok) {
    //       setAddDataModel(false);
    //       getDataHandler();
  ...
  const modalDeleteHandle = async () => {
    await MasterApis.DeleteApprovalTypes(SelectedId)
      .then((resp) => {
        getDataHandler();
        setSelectedId("");
        setShowDelete(false);
  ...
  return (
    <Fragment>
      <AddHeader btnName="Approval Type" addHandler={() => newAddHandler()} />

      <Col lg={12}>
        <Card>
          <Card.Header>
            <Card.Title>Approval Type</Card.Title>
          </Card.Header>
          <Card.Body>
                      ...
                      <tr key={item.id}>
                        ...
                        <td>
                          <div className="align-items-center">
                            {/* {item.logoImage} */}
                            <img
                              src={ConstantText.serverImagePath + item.logoImage}
