...
function DocumentType() {
  ...
  const getDataHandler = async () => {
    await MasterApis.GetDocumentTypes()
      .then((resp) => {
        if (resp.ok) {
          setData(resp.data);
        } else {
  ...
  const postData = async (values, resetForm) => {
    await MasterApis.PostDocumentTypes(values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
  ...
  const putData = async (values, resetForm) => {
    await MasterApis.PutDocumentTypes(SelectedId, values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
          setAddDataModel(false);
          getDataHandler();
  ...
  const modalDeleteHandle = async () => {
    await MasterApis.DeleteDocumentTypes(SelectedId)
      .then((resp) => {
        getDataHandler();
        setSelectedId("");
        setShowDelete(false);
...
//   put data prepare
  ...
  return (
    <Fragment>
      <AddHeader btnName="Add Document Type" addHandler={() => newAddHandler()} />

      <Col lg={12}>
        <Card>
          <Card.Header>
            <Card.Title>Document Type</Card.Title>
          </Card.Header>
          <Card.Body>
