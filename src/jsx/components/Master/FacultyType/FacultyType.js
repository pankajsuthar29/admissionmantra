...
function FacultyType() {
  ...
  const postData = async (values, resetForm) => {
    await MasterApis.PostFacultyTypes(values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
  ...
  const putData = async (values, resetForm) => {
    await MasterApis.PutFacultyTypes(SelectedId, values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
          setAddDataModel(false);
          getDataHandler();
  ...
  const modalDeleteHandle = async () => {
    await MasterApis.DeleteFacultyTypes(SelectedId)
      .then((resp) => {
        getDataHandler();
        setSelectedId("");
        setShowDelete(false);
...
//   put data prepare
  ...
  return (
    <Fragment>
      <AddHeader btnName="Add Faculty Type" addHandler={() => newAddHandler()} />

      <Col lg={12}>
        <Card>
          <Card.Header>
            <Card.Title>Faculty Type</Card.Title>
          </Card.Header>
          <Card.Body>
