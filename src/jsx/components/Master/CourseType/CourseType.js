...
function CourseType() {
  ...
  const postData = async (values, resetForm) => {
    await MasterApis.PostCourseTypes(values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
  ...
  const putData = async (values, resetForm) => {
    await MasterApis.PutCourseTypes(SelectedId, values.name, values.isActive)
      .then((resp) => {
        if (resp.ok) {
          setAddDataModel(false);
          getDataHandler();
  ...
  const modalDeleteHandle = async () => {
    await MasterApis.DeleteCourseTypes(SelectedId)
      .then((resp) => {
        getDataHandler();
        setSelectedId("");
        setShowDelete(false);
...
//   put data prepare
  ...
  return (
    <Fragment>
      <AddHeader btnName="Add Course Type" addHandler={() => newAddHandler()} />

      <Col lg={12}>
        <Card>
          <Card.Header>
            <Card.Title>Course Type</Card.Title>
          </Card.Header>
          <Card.Body>
